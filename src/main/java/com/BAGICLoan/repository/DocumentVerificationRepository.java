...
public interface DocumentVerificationRepository extends JpaRepository<DocumentVerification, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(dv) > 0 THEN true ELSE false END FROM DocumentVerification dv WHERE dv.identityVerified = true AND dv.addressVerified = true AND dv.annualIncome >= 30000 AND dv.creditScore >= 700")
    boolean isHighLimitEligible();

    @Query("SELECT CASE WHEN COUNT(dv) > 0 THEN true ELSE false END FROM DocumentVerification dv WHERE dv.identityVerified = true AND dv.addressVerified = true AND dv.annualIncome >= 20000 AND dv.creditScore >= 600")
