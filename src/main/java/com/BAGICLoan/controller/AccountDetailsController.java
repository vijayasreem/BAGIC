...
public class AccountDetailsController {
    ...
    public ResponseEntity<AccountDetails> updateAccountDetails(@PathVariable Long userId, @RequestParam(required = false) String newParam) {
        if (userId == null || userId == 0) {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
        AccountDetails accountDetails = accountDetailsService.updateByUserId(userId, newParam);
        if (accountDetails != null) {
            return new ResponseEntity<>(accountDetails, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
